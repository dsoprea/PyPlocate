...
def test_unescape_glob_to_plain_string():
    ...
    plain = plocate.patterns.unescape_glob_to_plain_string(r"foo\.bar")
...
def test_unescape_rejects_trailing_backslash():
    ...
    with pytest.raises(ValueError, match="escape character"):
        plocate.patterns.unescape_glob_to_plain_string("foo\\")
...
def test_compile_pattern_substring():
    ...
    assert plocate.patterns.matches_pattern(compiled, "/tmp/.catalog-repository.yaml")
...
def test_compile_pattern_glob():
    ...
    assert plocate.patterns.matches_pattern(compiled, "example.yaml")
    assert not plocate.patterns.matches_pattern(compiled, "example.txt")
...
def test_compile_pattern_ignore_case():
    ...
    assert compiled.kind == plocate.patterns.PatternKind.GLOB
    assert plocate.patterns.matches_pattern(compiled, "/tmp/README.TXT", ignore_case=True)
...
def test_compile_pattern_regex():
    ...
    assert plocate.patterns.matches_pattern(compiled, "/tmp/.catalog-repository.yaml")
    assert not plocate.patterns.matches_pattern(compiled, "/tmp/.catalog-repository.yaml.bak")
